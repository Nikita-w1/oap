...
export const ratingsRepository = {
  ...
  async findById(id: string): Promise<Rating | undefined> {
    ...
    return get<Rating>(`SELECT ${selectRating} FROM Ratings WHERE id = ${ratingId};`);
  },

  async create(rating: Omit<Rating, "id">): Promise<Rating> {
    const result = await run(`
      INSERT INTO Ratings (resourceId, userId, value, createdAt)
      VALUES (${Number(rating.resourceId)}, ${Number(rating.userId)}, ${rating.value}, '${rating.createdAt}');
    `);
  ...
  async update(id: string, value: number): Promise<Rating | null> {
    const ratingId = Number(id);
    const result = await run(`UPDATE Ratings SET value = ${value} WHERE id = ${ratingId};`);
  ...
  async delete(id: string): Promise<boolean> {
    const ratingId = Number(id);
    const result = await run(`DELETE FROM Ratings WHERE id = ${ratingId};`);
