...
export async function seedDb() {
  ...
  await run(`INSERT OR IGNORE INTO Resources (id, title, url, type, description, author, ownerUserId, createdAt) VALUES
    (1, 'JavaScript Basics', 'https://developer.mozilla.org/', 'Article', 'Навчальний ресурс з JavaScript', 'MDN', 1, '${now}'),
    (2, 'Node.js Docs', 'https://nodejs.org/', 'Article', 'Офіційна документація Node.js', 'Node.js Team', 2, '${now}'),
    (3, 'SQLite Tutorial', 'https://www.sqlitetutorial.net/', 'Course', 'Короткий курс по SQLite', 'SQLite Tutorial', 1, '${now}'),
    (4, 'Express Guide', 'https://expressjs.com/', 'Article', 'Документація Express', 'Express', 2, '${now}'),
    (5, 'TypeScript Handbook', 'https://www.typescriptlang.org/docs/', 'Book', 'Довідник TypeScript', 'Microsoft', 3, '${now}');`);
