declare module "sqlite3" {
  ...
  export interface Database {
    ...
    all<T = Record<string, unknown>>(sql: string, params: unknown[], callback: (err: Error | null, rows: T[]) => void): void;
    all<T = Record<string, unknown>>(sql: string, callback: (err: Error | null, rows: T[]) => void): void;
    get<T = Record<string, unknown>>(sql: string, params: unknown[], callback: (err: Error | null, row: T | undefined) => void): void;
