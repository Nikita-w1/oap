...
import type { ApiError, CreateResourceDto, ResourceDto, ResourceType, SortBy, SortDir } from "./dtos.js";
...
const formTitle = el<HTMLHeadingElement>("formTitle");
const submitBtn = el<HTMLButtonElement>("submitBtn");
const cancelEditBtn = el<HTMLButtonElement>("cancelEdit");
const reloadBtn = el<HTMLButtonElement>("reloadBtn");
const searchInput = el<HTMLInputElement>("searchInput");
const filterType = el<HTMLSelectElement>("filterType");
const sortBy = el<HTMLSelectElement>("sortBy");
const sortDir = el<HTMLSelectElement>("sortDir");
...
const typeInput = el<HTMLSelectElement>("type");
const descInput = el<HTMLTextAreaElement>("desc");
const authorInput = el<HTMLInputElement>("author");

function normalizeError(e: unknown): ApiError {
  if (typeof e === "object" && e !== null && "status" in e) return e as ApiError;
...
function readForm(): CreateResourceDto {
  return {
    ...
    type: typeInput.value as ResourceType,
    description: descInput.value.trim(),
    author: authorInput.value.trim(),
  };
}

function validate(dto: CreateResourceDto): boolean {
  clearFieldErrors();
  let valid = true;
  ...
  try {
    ...
    if (Array.isArray(err.details)) {
      err.details.forEach((detail: any) => {
        if (detail?.field === "title") showFieldError("title", "titleError", detail.message);
        if (detail?.field === "url") showFieldError("url", "urlError", detail.message);
        if (detail?.field === "type") showFieldError("type", "typeError", detail.message);
        if (detail?.field === "description") showFieldError("desc", "descError", detail.message);
        if (detail?.field === "author") showFieldError("author", "authorError", detail.message);
      });
    }
  } finally {
...
function readFilters() {
  ...
  state.query.sortBy = sortBy.value as SortBy;
  state.query.sortDir = sortDir.value as SortDir;
  state.query.page = 1;
}
